...
int main()
{
    ...
    GLuint normalTexture, heightTexture;
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, heightTexture, 0);

    // Specify the draw buffers for the fragment shader
    ...
    GLuint plyShader = LoadShaders("Shaders/PhongShader.vert", "Shaders/PhongShader.frag");
    ...
    do {
        ...
        planeMesh.draw(V, Projection, M, lightPos, deltaTime, camera.getCameraDirection());
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        glBindTexture(GL_TEXTURE_2D, normalTexture);
        glUniform1i(glGetUniformLocation(plyShader, "normalTexture"), 4); // bind the texture to texture unit 1


        glActiveTexture(GL_TEXTURE5);
        glBindTexture(GL_TEXTURE_2D, heightTexture);
        glUniform1i(glGetUniformLocation(plyShader, "heightTexture"), 5); // bind the texture to texture unit 1
        glUseProgram(0);
